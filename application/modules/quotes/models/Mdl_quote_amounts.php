...
class Mdl_Quote_Amounts extends CI_Model
{
    ...
    public function calculate_discount($quote_id, $quote_total)
    {
        ...
        $total = (float)number_format($quote_total ?? 0, 2, '.', '');
        $discount_amount = (float)number_format($quote_data->quote_discount_amount ?? 0, 2, '.', '');
        $discount_percent = (float)number_format($quote_data->quote_discount_percent ?? 0, 2, '.', '');
