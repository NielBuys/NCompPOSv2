...
class Mdl_Invoice_Amounts extends CI_Model
{
    ...
    public function calculate_discount($invoice_id, $invoice_total)
    {
        ...
        $total = (float)number_format($invoice_total ?? 0, 2, '.', '');
        $discount_amount = (float)number_format($invoice_data->invoice_discount_amount ?? 0, 2, '.', '');
        $discount_percent = (float)number_format($invoice_data->invoice_discount_percent ?? 0, 2, '.', '');
